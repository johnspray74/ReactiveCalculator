...
namespace ProgrammingParadigms
{
    ...
    /// Note the sender can only be wired to one receiver (no fanout). The receiver can we wired to multiple senders (fanin allowed)
    /// Finally note that the sender can choose to have a list of event ports but this is not usual becasue it complicates the sender abstraction. But it would allow the sender to have fanout. Normally fanout is accomplished by the alternative method of using a connecter - see below.
    ...
    /// <summary>
    /// This second methiod of implementing the Event programming paradigm (using IEventB) is more complicated, but overcomes a limitation in the C# language in the cae where
    ...
    /// When the reciever is wired, xxxPostWiringInitialize will be called by the wiring engine.  e.g. always use this pattern of three lines of code when using IEventB:
    /// 
    ...
    /// 
    /// Note hoever that the sender can only implement one IEventB. You cant have the above two lines of code twice in one domain abstraction. So it you can't have multiple send ports using iEventB.
    /// For this reason, domain abstractions that are senders usually do not use IEventB, they use IEvent fields. 
    /// To connect a sender that uses an IEventfield to a receiver that uses an IEventB field, simple use a connector (see below)
    ...
    /// <summary>
    /// The primary reason for the existence of the connector is dicussed above in the explanation of the IEventB Interface.
    ...
    /// 2. It allows a sender that does not support fanout to be wired to multiple receivers (because the connector supports fanout). Furthermore, the receiver can be a mix of ones that implement IEvent and ones that have IEventB fields.
